...
//		this.productService.updateProduct();
	...
	public String profileDisplay(Model model) {
		...
		try
		{
			...
			ResultSet rst = stmt.executeQuery(); // Use executeQuery() instead of execute()
			if (rst.next()) {
	...
	public String updateUserProfile(@RequestParam("userid") int userid, @RequestParam("username") String username, @RequestParam("email") String email, @RequestParam("password") String password, @RequestParam("address") String address) {
	    try {
	        ...
	        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecommjava", "root", ""); // Establish connection
	        ...
	        int i = pst.executeUpdate(); // Execute the update statement
	        ...
	        if (i > 0) {
	            ...
	            // Update failed
	            // Handle the failure case, maybe log something or redirect appropriately
	        }
	        
	        con.close(); // Close the connection after use
	    } catch (SQLException e) {
	        // Log the SQL exception
	        e.printStackTrace(); // Temporary logging, replace with proper logging mechanism
	    } catch (Exception e) {
	        // Handle any other exceptions that might occur
	        e.printStackTrace(); // Temporary logging, replace with proper logging mechanism
	    }
	    return "redirect:/index"; // Default redirection if an error occurs
	}
